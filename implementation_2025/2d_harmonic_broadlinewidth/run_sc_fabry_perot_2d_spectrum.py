...
reflection_monitor_location = mp.Vector3(layer_centers[0] - layer_thicknesses[0] / 2 + pml_thickness*1.2, 0., 0)
...
size_monitor = 0.1
incident_region_x = mp.FluxRegion(
        center=reflection_monitor_location, size=mp.Vector3(size_monitor, cell_y_size, mp.inf),
        weight=1.0, direction=mp.X
    )
incident_flux_monitor_x = sim.add_flux(
        frequency_probe, frequency_width, number_frequencies, incident_region_x
    )

incident_region_y = mp.FluxRegion(
        center=reflection_monitor_location, size=mp.Vector3(size_monitor, cell_y_size, mp.inf),
        weight=1.0, direction=mp.Y
    )
incident_flux_monitor_y = sim.add_flux(
        frequency_probe, frequency_width, number_frequencies, incident_region_y
    )
...
incident_flux_to_subtract_x = sim.get_flux_data(incident_flux_monitor_x)
incident_flux_to_subtract_y = sim.get_flux_data(incident_flux_monitor_y)
...
transmission_region_x = mp.FluxRegion(
        center=transmission_monitor_location, size=mp.Vector3(size_monitor, cell_y_size, mp.inf),
        weight=1.0, direction=mp.X
    )
transmission_flux_monitor_x = sim.add_flux(
        frequency_probe, frequency_width, number_frequencies, transmission_region_x
    )
reflection_region_x = incident_region_x
reflection_flux_monitor_x = sim.add_flux(
        frequency_probe, frequency_width, number_frequencies, reflection_region_x
    )
transmission_region_y = mp.FluxRegion(
        center=transmission_monitor_location, size=mp.Vector3(size_monitor, cell_y_size, mp.inf),
        weight=1.0, direction=mp.Y
    )
transmission_flux_monitor_y = sim.add_flux(
        frequency_probe, frequency_width, number_frequencies, transmission_region_y
    )
reflection_region_y = incident_region_y
reflection_flux_monitor_y = sim.add_flux(
        frequency_probe, frequency_width, number_frequencies, reflection_region_y
    )
sim.load_minus_flux_data(reflection_flux_monitor_x, incident_flux_to_subtract_x)
sim.load_minus_flux_data(reflection_flux_monitor_y, incident_flux_to_subtract_y)
...
# calculate the polariton decay EM energy dynamics and save the data
...
# calculate the spectrum and save the data
freqs_output = mp.get_flux_freqs(reflection_flux_monitor_x)

incident_flux_x = np.array(mp.get_fluxes(incident_flux_monitor_x))
transmitted_flux_x = np.array(mp.get_fluxes(transmission_flux_monitor_x))
reflected_flux_x = np.array(mp.get_fluxes(reflection_flux_monitor_x))

incident_flux_y = np.array(mp.get_fluxes(incident_flux_monitor_y))
transmitted_flux_y = np.array(mp.get_fluxes(transmission_flux_monitor_y))
reflected_flux_y = np.array(mp.get_fluxes(reflection_flux_monitor_y))

# we are interested in the direction normal to the incident plane-wave pulse
reflectance_x = -(reflected_flux_x) / (incident_flux_x)
reflectance_y = -(reflected_flux_y) / (incident_flux_y)
transmittance_x = (transmitted_flux_x) / (incident_flux_x)
transmittance_y = (transmitted_flux_y) / (incident_flux_y)
loss_x = 1.0 - reflectance_x - transmittance_x
loss_y = 1.0 - reflectance_y - transmittance_y

reflectance_sum = -(reflected_flux_x + reflected_flux_y) / (incident_flux_x + incident_flux_y)
transmittance_sum = (transmitted_flux_x + transmitted_flux_y) / (incident_flux_x + incident_flux_y)
loss_sum = 1.0 - reflectance_sum - transmittance_sum

# save the data
data_freq = np.zeros((len(freqs_output), 12))
data_freq[:, 0] = freqs_output
data_freq[:, 1] = loss_x
data_freq[:, 2] = reflectance_x
data_freq[:, 3] = transmittance_x
data_freq[:, 4] = loss_y
data_freq[:, 5] = reflectance_y
data_freq[:, 6] = transmittance_y
data_freq[:, 7] = loss_sum
data_freq[:, 8] = reflectance_sum
data_freq[:, 9] = transmittance_sum
data_freq[:, 10] = incident_flux_x
data_freq[:, 11] = incident_flux_y
header="Angle: %.4f, freq, loss_x, reflectance_x, transmittance_x, loss_y, reflectance_y, transmittance_y, loss_sum, reflectance_sum, transmittance_sum, incidence_x, incidence_y (in meep units)" %incident_angle
np.savetxt("spectrum.txt", data_freq, header=header, fmt="%.6E")

'''
# plot the EM field distribution at the end of the simulation
import matplotlib.pyplot as plt
fig, ax = plt.subplots()
sim.plot2D(fields=mp.Ez, ax=ax)
plt.show()
